...
export class TypeOperationComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    this.typeOperationService
      ...
      .subscribe(
        (res: HttpResponse<ITypeOperation[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: ITypeOperation): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInTypeOperations(): void {
    this.eventSubscriber = this.eventManager.subscribe('typeOperationListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITypeOperation[] | null, headers: HttpHeaders, page: number): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.router.navigate(['/type-operation'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
