...
export class OperationService {
  ...
  protected convertDateFromClient(operation: IOperation): IOperation {
    const copy: IOperation = Object.assign({}, operation, {
      dateOperation: operation.dateOperation && operation.dateOperation.isValid() ? operation.dateOperation.format(DATE_FORMAT) : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((operation: IOperation) => {
        operation.dateOperation = operation.dateOperation ? moment(operation.dateOperation) : undefined;
      });
    }
    return res;
  }
